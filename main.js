...
function analyzeConnections(manifest) {
  ...
  if (manifest.permissions) {
    connections.permissions = manifest.permissions.filter(
...
function analyzeIntegrations(manifest) {
  ...
  if (manifest.background) {
    if (manifest.background.persistent !== undefined) {
      integrations.push(
        `Background: ${manifest.background.persistent ? "Persistent" : "Non-persistent"}`,
      );
    }
  ...
  if (manifest.commands) {
    integrations.push(
      `Commands: ${Object.keys(manifest.commands).length} registered`,
    );
  }
...
async function scanExtension(extension) {
  ...
  console.log(
    gradientColors.green +
      `│ Scanning: ${extension.name}` +
      " ".repeat(Math.max(0, 38 - extension.name.length)) +
      gradientColors.reset +
      gradientColors.cyan +
      "│" +
      gradientColors.reset,
  ...
  console.log(
    gradientColors.yellow +
      `│ ID: ${extension.id}` +
      " ".repeat(Math.max(0, 38 - extension.id.length)) +
      gradientColors.reset +
      gradientColors.cyan +
      "│" +
      gradientColors.reset,
  ...
  console.log(
    gradientColors.yellow +
      ...
      " ".repeat(Math.max(0, 38 - extension.version.length)) +
      gradientColors.reset +
      gradientColors.cyan +
      "│" +
      gradientColors.reset,
  ...
  if (
    ...
    if (extension.manifest.permissions.length > 10) {
      console.log(
        gradientColors.blue +
          `  • ... and ${extension.manifest.permissions.length - 10} more` +
          gradientColors.reset,
      );
    }
  } else {
  ...
  if (vtResult.error) {
    console.log(
      gradientColors.yellow + `  ⚠️  ${vtResult.error}` + gradientColors.reset,
    );
  } else {
    console.log(
      gradientColors.green +
        `  ✅ Score: ${vtResult.score}/100` +
        gradientColors.reset,
    );
  }
...
async function main() {
  ...
  extensions.forEach((ext, index) => {
    ...
    console.log(
      gradientColors.blue + `     ID: ${ext.id}\n` + gradientColors.reset,
    );
  });

  rl.question(
    gradientColors.cyan +
      "\n🔍 Select extension number to scan: " +
      gradientColors.reset,
    async (answer) => {
